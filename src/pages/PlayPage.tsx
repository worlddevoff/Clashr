...
import type { LobbyRoom } from '../types/domain';
...
export function PlayPage() {
  ...
  const [createCap, setCreateCap] = useState<PartyCapacity>(5);
  ...
  useEffect(
    () =>
      subscribePublicParties((list) =>
        setPublicParties(
          list.filter((party) => !party.gameSlug || party.gameSlug === 'bomb-party'),
        ),
      ),
    [],
  );

  const joinQuick = (room: LobbyRoom) => {
    setError(null);
  ...
  const createBombParty = (visibility: PartyVisibility, capacity = createCap) => {
    if (!user) {
      setError('Connect a wallet to create a SOL party.');
      return;
    }
    setError(null);
    ...
    navigate(
      `/party/${party.id}?game=bomb-party&cap=${party.capacity}&host=${encodeURIComponent(party.hostId)}&vis=${visibility}&stake=${entryLamports}`,
    );
  };
  ...
  const joinPublic = (listing: PublicPartyListing) => {
    ...
    navigate(
      `/party/${listing.id}?game=bomb-party&cap=${listing.capacity}&host=${encodeURIComponent(listing.hostId)}&vis=public&stake=${stake}`,
    );
  };

  const joinByCode = () => {
    if (!user) {
      setError('Connect a wallet to join a Bomb Party party.');
      return;
    }
    const code = parsePartyCode(joinCode);
    if (!code) {
      setError('Enter a valid Bomb Party party code.');
      return;
    }
    navigate(`/party/${code}?game=bomb-party&cap=5`);
  };
    ...
    <div className="mx-auto max-w-6xl px-4 py-8 sm:px-6">
      <div className="mb-6 flex flex-wrap items-center justify-between gap-3">
        <div>
          ...
          <p className="mt-1 text-sm text-white/55">
            Practice vs bots for free, or stake SOL in a public or private party — same escrow as Tower.{' '}
      ...
      <section className="mb-10">
        ...
        <p className="mt-2 text-sm text-white/50">
          Public and private Bomb Party parties use the same on-chain escrow as Tower. Every connected
          wallet stakes before the host can start; bots never stake.
        </p>
      ...
      {publicParties.length === 0 ? (
        <div className="mb-10 rounded-2xl border border-dashed border-ink-600 bg-ink-900/40 px-5 py-8 text-center">
          ...
          <p className="mt-2 text-sm text-white/45">No public Bomb Party lobbies yet.</p>
          <Button
            className="mt-4"
            variant="secondary"
            disabled={!user}
            onClick={() => createBombParty('public')}
          >
            ...
            return (
              ...
              >
                ...
                <Button
                  ...
                  onClick={() => joinPublic(listing)}
                >
                  {seats <= 0 ? 'Full' : 'Join Bomb Party lobby'}
                </Button>
              </motion.div>
            );
          })}
      ...
      <div className="mb-4">
        ...
        <p className="mt-0.5 text-xs text-white/35">
          Practice vs bots is free. Real SOL pots need 2+ wallets.
        </p>
      </div>
          ...
          >
            ...
            <div className="flex flex-1 flex-col gap-3 p-5">
              ...
              <div className="mt-auto flex flex-col gap-2">
                <Button className="w-full" onClick={() => joinQuick(room)}>
                  Play solo
                </Button>
                <Button
                  className="w-full"
                  variant="secondary"
                  disabled={!user}
                  onClick={() => createBombParty('private', room.capacity as PartyCapacity)}
                >
